...
public class MemorySearchService extends Service<SearchResult> implements IMessageArguments {
    ...
    private class SearchTask extends Task<SearchResult> {
        ...
        private void update(long size) {
            ...
            int tail = (int)(Math.random()*100);
            updateMessage(String.format("search.service.refine~%03d", tail));
        ...
        private DumpRegionSupplier computeRegions(SearchResult prev) {
            ...
            MemoryInfo[] infos = conn.query(prev.getStart(), 14000);
        ...
        MemoryDump createDump(SearchResult res, DumpRegionSupplier supplier) throws IOException {
            ...
            try {
                ...
                dump.getInfos().addAll(Arrays.asList(conn.query(0, 14000)));
            ...
            while (!isCancelled()) {
                ...
                while (size > 0 && !isCancelled()) {
                    ...
                    int tail = (int)(Math.random()*100);
                    updateMessage(String.format("search.service.dumping~%03d", tail));
